...
class ArticleFinderServiceTest extends TestCase{
    ...
    public function test_getArticles_shouldReturnInstancesOfArticle_whenFinderHasFoundDataFromFinder()
    {
        $this->finder->expects($this->any())->method('__invoke')->willReturn($this->articles);
        $this->service->findArticles($this->finder);
        $this->assertInstanceOf(Article::class, $this->service->getArticles()[0]);
    }
    
    
    public function test_getFirstOrThrow_shouldThrowException_whenGetFirstReturnNull(){
        $this->finder->expects($this->any())->method('__invoke')->willReturn([]);
        $this->service->findArticles($this->finder);
        
        $this->expectException(ArticleFinderException::class);
        $this->service->getFirstOrThrow();
    }
